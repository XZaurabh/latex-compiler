...
compileBtn.addEventListener('click', async () => {
  ...
  const targetUrl = `https://latexonline.cc/compile?text=${encodeURIComponent(code)}`;
  ...
  for (let i = 0; i < proxies.length; i++) {
    ...
    try {
      ...
      addLogLine(`! Proxy ${i + 1} failed: ${error.message}`);
      // Continue to next proxy
    }
  }

  if (!success) {
    lastErrorLog = 'Network error: All proxies failed.';
    addLogLine('! All proxies failed. Host this file on an HTTP server.');
